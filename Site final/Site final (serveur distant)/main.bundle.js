...
var AppComponent = (function () {
    function AppComponent(http) {
        ...
        //Les galeries
        this.http.get('http://137.74.114.220:80/api/piles/').subscribe(function (res) {
            self.infoGaleries = res.json().data[0].piles;
            self.nbGal = self.infoGaleries.length;
            self.correction();
        });
        // Les descriptions d'auteurs
        this.http.get('http://137.74.114.220:80/api/auteurs/').subscribe(function (res) {
            self.infoAuteurs = res.json().data[0].auteurs;
        });
        // Les liens planetes
        this.http.get('http://137.74.114.220:80/api/planete/').subscribe(function (res) {
            self.infoPlanetes = res.json().data[0].planetes;
        });
        // Les descriptions des galeries
        this.http.get('http://137.74.114.220:80/api/galeries/').subscribe(function (res) {
